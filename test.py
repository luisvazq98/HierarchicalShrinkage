...
if __name__ == "__main__":
    ...
    # Separate results for each model type
    ...
    cart_avg_depth = cart.groupby('Max Leaves')['Max Depth'].mean().reset_index()
    hs_avg_depth = hscart.groupby('Max Leaves')['Max Depth'].mean().reset_index()

    cart_avg_nodes = cart.groupby('Max Leaves')['Node Count'].mean().reset_index()
    hs_avg_nodes = hscart.groupby('Max Leaves')['Node Count'].mean().reset_index()


    # Add a 'Model' column to each dataframe
    cart_avg_depth['Model'] = 'CART'
    hs_avg_depth['Model'] = 'HSCART'

    cart_avg_nodes['Model'] = 'CART'
    hs_avg_nodes['Model'] = 'HSCART'

    # Combine the dataframes
    combined_depth = pd.concat([cart_avg_depth, hs_avg_depth], ignore_index=True)
    combined_nodes = pd.concat([cart_avg_nodes, hs_avg_nodes], ignore_index=True)





    # Create violin plots for Maximum Depth and Node Count across all seeds per model type
    fig, axs = plt.subplots(1, 2, figsize=(14, 6))

    # Violin plot for Maximum Depth by Model type
    sns.violinplot(x='Model', y='Max Depth', data=combined_depth, ax=axs[0])
    axs[0].set_title('Distribution of Maximum Depth')
    axs[0].grid(True)

    # Violin plot for Node Count by Model type
    sns.violinplot(x='Model', y='Node Count', data=combined_nodes, ax=axs[1])
    axs[1].set_title('Distribution of Node Count')
    axs[1].grid(True)
